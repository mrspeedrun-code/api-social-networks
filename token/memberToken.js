...
class JWT {
   ...
   */
  saveToken (token) {
    if (process.env.TOKENS_MEMBER) {
      const tokens = process.env.TOKENS_MEMBER.split(',')

      tokens.push(token)

      process.env.TOKENS_MEMBER = tokens.join(',')

      return
    }

    process.env.TOKENS_MEMBER = token
  }

  /**
   * Get tokens
   * @return {Array} TOKENS_MEMBER
   */
  getTokens () {
    return process.env.TOKENS_MEMBER.split(',')
  }
  ...
  verifyMemberToken () {
    process.env.TOKENS_MEMBER = 'membertoken1234'

    return (req, res, next) => {
      if (req.headers['access-token-member'] || req.headers['token']) {
        const token = req.headers['access-token-member'] || req.headers['token']
      ...
      res.status(403).json({
        code: 403,
        message: 'Invalid parameters in header please set token or access-token-member'
      })
    }
  }
}
