...
class JWT {
   ...
   */
  saveToken (token) {
    if (process.env.TOKENS_ORGANIZER) {
      const tokens = process.env.TOKENS_ORGANIZER.split(',')

      tokens.push(token)

      process.env.TOKENS_ORGANIZER = tokens.join(',')

      return
    }

    process.env.TOKENS_ORGANIZER = token
  }

  /**
   * Get tokens
   * @return {Array} TOKENS_ORGANIZER
   */
  getTokens () {
    return process.env.TOKENS_ORGANIZER.split(',')
  }
  ...
  verifyOrganizerToken () {
    process.env.TOKENS_ORGANIZER = 'organizertoken1234'

    return (req, res, next) => {
      if (req.headers['access-token-organizer'] || req.headers['token']) {
        const token = req.headers['access-token-organizer'] || req.headers['token']
      ...
      res.status(403).json({
        code: 403,
        message: 'Invalid parameters in header please set token or access-token-organizer'
      })
    }
  }
}
